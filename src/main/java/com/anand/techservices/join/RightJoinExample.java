...
public class RightJoinExample {
    public static void main(String[] args) {
                ...
                for (String user: userTable) {
                    if (orderTable.iterator().hasNext()) {
                        for (String order: orderTable) {
                            ctx.output(key + "," + user + "," + order);
                        }
                    } else {
                        ctx.output(key + "," + user + ",null, null, null");
                    }
        ...
        output.apply(TextIO.write().to("/Users/anand/Documents/Amit/projects/apache-beam-training-2/src/main/resources/output/right_join_example.csv").withNumShards(1).withSuffix(".csv"));
